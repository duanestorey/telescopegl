...
Run:
    python -m telescope.tests.benchmark_em [--repeat N] [--output FILE] [--backend stock|auto]
"""
...
def run_benchmark(backend_mode='auto', repeat=3):
    ...
    print('=' * 80)
    print(f'Telescope Benchmark — Backend: {active.name}')
    ...
    seed = ts.get_random_seed()
    np.random.seed(seed)
    tl2 = TelescopeLikelihood(ts.raw_scores, opts)
    inum = 0
    ...
    seed = ts.get_random_seed()
    np.random.seed(seed)
    tl3 = TelescopeLikelihood(ts.raw_scores, opts)
    n_detail = 10
    estep_times = []
    mstep_times = []
    lnl_times = []
...
def main():
    parser = argparse.ArgumentParser(description='Telescope multi-tier benchmark')
