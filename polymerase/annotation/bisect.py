...
class _AnnotationBisect:
    ...
    def __init__(self, gtffile, min_overlap=None, attr_name='locus'):
        ...
        # GTF filehandle
        fh = open(gtffile) if isinstance(gtffile, str) else gtffile  # noqa: SIM115
        features = (GTFRow(*line.strip('\n').split('\t')) for line in fh if not line.startswith('#'))
        for i, f in enumerate(features):
            attr = dict(re.findall(r'(\w+)\s+"(.+?)";', f.attribute))
            _locus_name = attr.get(self.key, f'TELE{i:04d}')
    ...
    def intersect_blocks(self, ref, blocks):
        ...
        _result = Counter()  # noqa: F841 — unreachable, placeholder
        for _b_start, _b_end in blocks:
            pass
        return _result
