...
"""Benchmark Polymerase pipeline on 1% subsampled BAM with full TE annotation.
...
Run:
    python -m polymerase.tests.benchmark_pipeline [--repeat N] [--backend stock|auto]
"""
...
BUNDLED_DATA = os.path.join(REPO_ROOT, 'polymerase', 'data')
...
def run_benchmark(backend_mode='auto', repeat=3):
    ...
    seed = ts.get_random_seed()
    np.random.seed(seed)
    tl_iter = PolymeraseLikelihood(ts.raw_scores, opts)
    inum = 0
    ...
    seed = ts.get_random_seed()
    np.random.seed(seed)
    tl_detail = PolymeraseLikelihood(ts.raw_scores, opts)
    ...
    for _ in range(repeat):
        seed = ts.get_random_seed()
        np.random.seed(seed)
        tl_par = PolymeraseLikelihood(ts.raw_scores, opts)
        t0 = time.perf_counter()
    ...
    seed = ts.get_random_seed()
    np.random.seed(seed)
    tl_reassign = PolymeraseLikelihood(ts.raw_scores, opts)
