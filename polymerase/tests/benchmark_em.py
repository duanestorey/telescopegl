...
def print_table(results, reference=REFERENCE):
    ...
    border = '+' + '-' * 29 + '+' + '-' * 15 + '+' + '-' * 15 + '+' + '-' * 18 + '+'
    header = '| {:27s} | {:13s} | {:13s} | {:16s} |'.format('Stage', 'Current', 'Reference', 'Speedup')
    ...
    rows = [
        ...
        ('Memory peak', f'{results["memory_peak_mb"]:.2f} MB', f'{reference["memory_peak_mb"]:.2f} MB', ''),
    ]

    for stage, current, ref, speedup in rows:
        print(f'| {stage:27s} | {current:13s} | {ref:13s} | {speedup:16s} |')
        print(border)
