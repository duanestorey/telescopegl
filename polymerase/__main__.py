...
def generate_test_command(args):
    ...
    if not os.path.exists(_alnpath):
        raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), _alnpath)
    if not os.path.exists(_gtfpath):
        raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), _gtfpath)
    print(f'polymerase assign {_alnpath} {_gtfpath}', file=sys.stdout)
...
def main():
    ...
    list_plugins_parser = subparser.add_parser(
        'list-plugins',
