...
from ..utils.helpers import str2int, region_iter, phred, merge_overlapping_regions
...
class Polymerase(object):
    ...
    def __str__(self):
        if hasattr(self.opts, 'samfile'):
            return '<Polymerase samfile=%s, gtffile=%s>'.format(
                self.opts.samfile, self.opts.gtffile)
        elif hasattr(self.opts, 'checkpoint'):
            return '<Polymerase checkpoint=%s>'.format(self.opts.checkpoint)
        else:
            return '<Polymerase>'
