...
"""PolymeraseLikelihood — EM algorithm for resolving multi-mapped reads.

This module contains the core EM algorithm used by Polymerase to assign
...
class PolymeraseLikelihood:
    ...
    def em(self, use_likelihood=False, loglev=lg.WARNING, save_memory=True):
        ...
        while not (converged or reached_max):
            ...
            lg.debug(f'time: {perf_counter() - xtime}')
        ...
        lg.log(loglev, f'EM {_con:s} after {inum:d} iterations.')
        lg.log(loglev, f'Final log-likelihood: {self.lnl:f}.')
        return

    # TODO (future): Parallel reassignment — run the 6 reassignment modes
    # concurrently via ThreadPoolExecutor. Low priority since reassignment
    # is currently ~0.1s even on full datasets.
    ...
    def _from_block(cls, sub_matrix, parent):
        """Create a sub-PolymeraseLikelihood from a block of the parent's matrix.

        Args:
            sub_matrix: Sub-block sparse score matrix.
            parent: Parent PolymeraseLikelihood instance (for opts).

        Returns:
            New PolymeraseLikelihood instance for the sub-block.
        """
    ...
    def em_parallel(self, use_likelihood=False, loglev=lg.WARNING, n_workers=None):
        ...
        from ..sparse.decompose import find_blocks, merge_results, split_matrix

        n_components, labels = find_blocks(self.raw_scores)
        lg.log(loglev, f'Block decomposition: {n_components} components')
        ...
        if backend.threadsafe_parallelism:
            ...
            lg.log(loglev, f'Parallel EM: {_n} workers for {n_components} blocks')
