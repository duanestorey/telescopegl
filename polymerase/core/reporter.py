...
def output_report(polymerase_obj, tl, stats_filename, counts_filename):
    """Generate TSV stats and counts reports.

    Args:
        polymerase_obj: Polymerase instance with feat_index, feature_length, run_info, opts
    ...
    """
    _rmethod = polymerase_obj.opts.reassign_mode
    _rprob = polymerase_obj.opts.conf_prob
    _fnames = sorted(polymerase_obj.feat_index, key=polymerase_obj.feat_index.get)
    _flens = polymerase_obj.feature_length
    ...
    _comment = ["## RunInfo"]
    _comment += ['{}:{}'.format(*tup) for tup in polymerase_obj.run_info.items()]
...
def update_sam(polymerase_obj, tl, filename):
    """Update SAM file with EM-derived assignments and quality scores.

    Args:
        polymerase_obj: Polymerase instance with tmp_bam, read_index, feat_index, run_info, opts
        tl: PolymeraseLikelihood instance (after EM convergence)
    ...
    _threads = getattr(polymerase_obj.opts, 'ncpu', 1)
    with pysam.AlignmentFile(polymerase_obj.tmp_bam, check_sq=False,
                             threads=_threads) as sf:
        header = sf.header
        header['PG'].append({
            'PN': 'polymerase', 'ID': 'polymerase',
            'VN': polymerase_obj.run_info['version'],
            'CL': ' '.join(sys.argv),
        })
        ...
        for code, pairs in alignment.fetch_fragments_seq(sf, until_eof=True):
            if len(pairs) == 0:
                continue
            ridx = polymerase_obj.read_index[pairs[0].query_id]
            for aln in pairs:
                if aln.is_unmapped:
                    aln.write(outsam)
                    continue
                ...
                if aln.r1.get_tag('ZT') == 'SEC':
                    ...
                    fidx = polymerase_obj.feat_index[aln.r1.get_tag('ZF')]
                    prob = tl.z[ridx, fidx]
