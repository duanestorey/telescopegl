...
def run(args, sc=True):
    ...
    from ..plugins.snapshots import AlignmentSnapshot, AnnotationSnapshot
    ...
    lg.info(f'Loaded {len(annot.loci)} features.')
    console.verbose(f'Loaded annotation: {len(annot.loci):,} features ({_annot_elapsed:.1f}s)')
    ...
    sw.stop()
    lg.info(f'Loaded alignment in {fmtmins(_aln_elapsed)}')
    ...
    console.status(f'Loading alignment... done ({_aln_elapsed:.1f}s)')
    console.detail(
        f'{_total:,} fragments \u2014 {_overlap:,} overlap annotation ({_unique:,} unique, {_ambig:,} ambiguous)'
    )
    console.blank()

    if _overlap == 0:
        console.status('No alignments overlapping annotation.')
        lg.info(f'polymerase assign complete ({fmtmins(time() - total_time)})')
        return
    ...
    lg.debug(f'garbage: {gc.collect():d}')
